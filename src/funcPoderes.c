...
void setVeriavel(listaQuestao *questionario){
    ...
    questionario->tempo     = 15;

    //quantidade de acertos para elimitar fantasma
    questionario->QntdRespostaMatarFantasma = 2;

    //total de pontos inicial para jogador eliminar fantasma --- 0/5 
    questionario->matarFantasma     = 0;

    //quantidades de acertos e erros no jogo
    questionario->acertos = 0;
    questionario->erros = 0;
}
...
void acertou(listaQuestao *questionario, bonecoJog *jogador){
    ...
    if (questionario->tempo >= 20){
        questionario->tempo     = 20;
    }

    jogador->forca          += 0.37;
    questionario->acertos   += 1;
    questionario->drawResp   = 1;   // flag indica 0 para respota errada utilizada para desenhar na tela
...
void errou(listaQuestao *questionario){
    ...
    questionario->erros    += 1;
    questionario->drawResp  = 0; // flag indica 0 para respota errada utilizada para desenhar na tela
...
void cronometroRegressivo(listaQuestao *questionario, int *sair){
...
void drawResposdeu(ALLEGRO_FONT *fonte_equacao, listaQuestao *questionario, telaAlternativa *posicaoAlt){
    ...
    switch(questionario->drawResp){
        case 1:   
            al_draw_text(fonte_equacao, al_map_rgb(157, 11 , 43), posicaoAlt->resultado_x, posicaoAlt->resultado_y - 40, ALLEGRO_ALIGN_CENTRE, "Acertou");
            break;

        case 0:
            al_draw_text(fonte_equacao, al_map_rgb(157, 11 , 43), posicaoAlt->resultado_x, posicaoAlt->resultado_y - 40, ALLEGRO_ALIGN_CENTRE, "Errou");
            break;
        
        case -1:
            al_draw_text(fonte_equacao, al_map_rgb(157, 11 , 43), posicaoAlt->resultado_x, posicaoAlt->resultado_y - 40, ALLEGRO_ALIGN_CENTRE, "Começou");
            break;
    }

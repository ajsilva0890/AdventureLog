...
#include "Equacoes_aleatorias/equacoes.h"
#include "structs.h"
...
int main(void)
{
    ...
    while (!sair){
	    ...
	    if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP){
	    	if (evento.mouse.x > meio_resp_A-200 && 
                evento.mouse.x < meio_resp_A+200 &&
...
int main(void)
{
    ...
    while (!sair){
        ...
        if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP){
            ...
            if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP){
                if (evento.mouse.x > meio_resp_C-20 && 
                    evento.mouse.x < meio_resp_C+20 &&
                    evento.mouse.y > (ALTURA_TELA/3)*2 - 20 &&
                    evento.mouse.y < (ALTURA_TELA/3)*2 + 20 ){
